<template>
   <LoadingView :title="title" :items="items" :loading="loading">
      <form v-if="form" @submit.prevent="saveData">
         <!-- Image -->
         <image-upload v-model="form.image.data" :url="form.image.url" :validation="$v.form.image.data" :label="$t('label.image')"></image-upload>
         <!-- Caption -->
         ...
         <!-- Conditions -->
         <input-details label="Conditions" labelClasss="text-sm-right">
...
export default {
   ...
   data() {
      return {
         title: "menuitems.offers.list.edit",
         items: [{ text: "menuitems.offers.list.edit", active: true }],
         loading: true,
         form: null,
         ...
         typeOptions: [
            { name: "Free Product", id: "FREE_PRODUCT" },
   ...
   computed: {
      conditionfields() {
         return this.form.conditions.map(con => {
            return [
               {
               ...
               },
               {
                  ...
                  label: 'Value',
                  component: con.type == 'MINIMUM_ORDER_VALUE' ? "input-control" : 'select-field', // Render component type
                  value: "", // Initial value                  
                  props: {
                     type: "text",
                     options: con.type == 'DAY' ? this.days : con.type == 'CATEGORY_ID' ? this.categories : null
                  },
               },
            ]
         })
   ...
   methods: {
      async saveData(isContinue) {
         if (this.form.type == "DISCOUNT") delete this.form.freeProductId
         if (this.form.type == "FREE_PRODUCT") delete this.form.discountValue
   ...
   async created() {
      try {
         const resOffer = await this.axios.get(
            `/menu/offers/${this.$route.params.id}`
         )
         this.form = resOffer.data.payload
         const resCategory = await this.axios.get("/menu/categories")
