...
export default {
  ...
  async created() {
    const user = await this.axios.get(
      `/users/${window.jara.getCookie("userid")}`
    );
    this.user = user.data.payload;
  },
  components: { simplebar },
  mounted() {
    //  setInterval(() => {
    //    const pending = this.axios.get(`/checkout/pending/1`);
    ...
    <div class="navbar-header">
      ...
      <div class="d-flex">
            ...
            >
              <div class="media">
                ...
                <div class="media-body">
                  <!-- <h6 class="mt-0 mb-1">
                    {{ item.content.name }} placed an order
                  </h6> -->
        ...
        >
          <template v-slot:button-content>
            <img
              v-if="user.meta && user.meta.user_image"
              class="rounded-circle header-profile-user"
              :src="user.meta.user_image"
              alt="Header Avatar"
            />
